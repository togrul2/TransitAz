...
for(let i = 0; i < data.length; i++) {
    ...
    // for(let j = 0; j < JSON.parse(data[i].seats).length; j++){
    //     // console.log(JSON.parse(data[i].seats)[j], data[i].seats_selected)
    //
    //     if (data[i].seats_selected.includes(JSON.parse(data[i].seats)[j]))
    //         seat_btns_txt += `<button data-parent="${data[i].id}" class="seat-btn-${data[i].id} seat-btn btn active btn-outline-primary btn-sm m-1">${JSON.parse(data[i].seats)[j]}</button>`;
    //     else
    //         seat_btns_txt += `<button data-parent="${data[i].id}" class="seat-btn-${data[i].id} seat-btn btn btn-outline-primary btn-sm m-1">${JSON.parse(data[i].seats)[j]}</button>`;
    // }
        ...
        <div class="accordion-item">
            ...
            <div id="flush-collapse-${data[i].id}" class="accordion-collapse collapse" aria-labelledby="flush-heading-${data[i].id}" data-bs-parent="#tickets_accordion">
                <div class="accordion-body">
                    ...
                    <button class="delete-btn btn btn-danger" data-id="${data[i].id}">Delete</button>
                </div>
            </div>
        </div>
    `
